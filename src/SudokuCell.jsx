...
class SudokuCell extends Component{
  constructor(props) {
	...
	this.boundHandleOnKeyUp = this.handleOnKeyUp.bind(this);
  ...
  // Some browsers can't handle onkeyup and some can't handle oninput so we write both event handlers
  // and set state variables to indicate that we're done handling all input events.
  handleOnKeyUp(e){
	if (this.state.editing){
		if ((e.nativeEvent.key === "Esc") || (e.nativeEvent.key === "Escape")) {
			this.setState({editing: false, revealNumber: false});
		} else if (parseInt(String(e.currentTarget.value).trim()) === parseInt(String(this.props.cellAnswer).trim())){
  ...
  render() {
	...
	if (this.state.editing){
		var inputElement = (<input type="number" length="1" value="" onKeyUp={this.boundHandleOnKeyUp} onBlur={this.boundHandleOnBlur} onFocus={this.boundHandleOnFocus} onInput={this.boundHandleOnInput} ref={this.inputRef}/>); 
		// eslint-disable-next-line no-console
